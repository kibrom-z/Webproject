...
if ((isset($_GET['id'])) && (is_numeric($_GET['id']))) {
  $trip_number = htmlspecialchars($_GET['id'], ENT_QUOTES);
} elseif ((isset($_POST['id'])) && (is_numeric($_POST['id']))) {
  $trip_number = htmlspecialchars($_POST['id'], ENT_QUOTES);
} else { // No valid ID, kill the script.
  print '<p>This page has been accessed in error.</p>';
  exit();
}

$_SESSION['resv-trip-id'] = $trip_number;
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="collapse navbar-collapse" id="navmenu">
          <ul class="navbar-nav ms-auto">
            <li class="nav-item fs-5 me-lg-5">
              <a href="select-trip-id.php" class="nav-link active">Reservations</a>
            </li>
            <li class="nav-item fs-5 me-lg-5">
    ...
    <div class="container">
      <div class="row w-75 m-auto mt-3">
        <div class="col bg-light rounded-3 p-5 text-center">
          <p class="fs-4 text-success mb-5">Reservations</p>
          <p class="text-start lead"><?php print '' . $_SESSION['departure-city'] . ' — ' . $_SESSION['arrival-city'] . ' (' . $_SESSION['departure-date'] . ')'; ?></p>
          <form action="reservations.php?id=<?php print $_SESSION['resv-trip-id']; ?>" method="post">
            <?php
            require('../../../php/reservation-tasks/process-passenger-attendance.php');
            ?>
            <a href="select-trip-id.php">
              <input type="button" name="back-button" value="GO BACK" id="back-button" class="fs-5 btn btn-success rounded-pill mt-4 py-1 px-5 me-lg-5">
            </a>
            <input type="submit" name="submit" value="SUBMIT ATTENDANCE" class="fs-5 btn btn-success rounded-pill mt-4 py-1 px-4 ms-lg-5" />
          </form>
        </div>
      </div>
    </div>
