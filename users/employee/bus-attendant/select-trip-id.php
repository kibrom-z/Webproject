...
<body>
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    require('../../../php/reservation-tasks/display-reservations.php');
  }
  ?>
    ...
    <div class="container">
      <div class="row w-50 m-auto mt-3">
        <div class="col bg-light rounded-3 p-5 text-center">
          <p class="fs-4 text-success mb-5">Select a trip id to view reservations</p>
          <form action="select-trip-id.php" method="post" class="small-container">
            <div class="input-group mb-3 w-75">
              <label for="trip-id" class="form-label w-35 pt-1 pe-3 text-end">Trip ID</label>
              <input type="number" class="form-control ps-4" name="trip-id" id="trip-id" min="0" value="<?php if (isset($_POST['trip-id'])) print $_POST['trip-id']; ?>" autofocus required style="width: 80px;" />
            </div>
            <input type="submit" name="get-res" id="get-res" value="GET RESERVATIONS" class="fs-5 btn btn-success rounded-pill mt-5 py-1 w-50" />
          </form>
        </div>
      </div>
    </div>
