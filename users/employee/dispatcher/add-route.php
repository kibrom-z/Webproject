...
<body>
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    require('../../../php/route-tasks/process-add-route.php');
  }
  ?>
      ...
      <div class="row w-50 m-auto mt-3">
        <div class="col bg-light rounded-3 p-5 text-center">
          ...
          <form action="add-route.php" method="post">
            <?php
            require('../../../php/route-tasks/load-cities.php');
            ?>
            <div class="input-group mb-3">
              <label for="distance" class="form-label w-35 pt-1 pe-3 text-end">Distance in km</label>
              <input type="number" class="form-control ps-4" name="distance" id="distance" min="0" max="2000" value="<?php if (isset($_POST['distance'])) print $_POST['distance']; ?>" required />
            </div>
            ...
            <input type="submit" name="add-route" id="add-route" value="ADD ROUTE" class="fs-5 btn btn-success rounded-pill mt-4 py-1 px-5 ms-lg-5" />
          </form>
        </div>
      </div>
    </div>
