...
<head>
  ...
  <title>Postpone — Available trips</title>
    ...
    <div class="container">
      <div class="row w-50 m-auto mt-3">
        <div class="col bg-light rounded-3 p-5 text-center">
          <p class="fs-4 text-success mb-5">Available Trips</p>
          <?php
          ...
          if ($id == 1) {
            require('../../php/booking-tasks/auxiliary/display-available-trips.php');
          } else {
            print '<p class="fs-5 mb-5">No trips are found. Change the date and try again.</p>';
          }
          print '
        </div>
        <div class="field">
          <a href="postpone.php?id=' . $_SESSION['postpone-reservation-id'] . '">
            <input type="button" name="back" id="back-button" value="GO BACK" class="fs-5 btn btn-success rounded-pill mt-5 py-1 w-35" /></a>
        </div>';
          ?>
        </div>
      </div>
