...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        // Two routes are deleted together (as they were created at once)
        ...
        // Deletes the selected route based on its id
        $query = "DELETE FROM routes WHERE route_id=? LIMIT 1";
        $q = mysqli_stmt_init($dbcon);
        mysqli_stmt_prepare($q, $query);
        mysqli_stmt_bind_param($q, 'i', $id);
        mysqli_stmt_execute($q);

        // Deletes the other route using the two infomation
        $third_query = "DELETE FROM routes WHERE departure_place=? AND arrival_place=? LIMIT 1";
        $third_q = mysqli_stmt_init($dbcon);
        mysqli_stmt_prepare($third_q, $third_query);
        mysqli_stmt_bind_param($third_q, 'ss', $second_arrival, $second_departure); // Crafty place switch
        mysqli_stmt_execute($third_q);

        if ((mysqli_stmt_affected_rows($q) == 1) && (mysqli_stmt_affected_rows($third_q) == 1)) { // Update OK
            header('location: ../../../php/route-tasks/successful-route-delete.php');
    ...
    if (mysqli_num_rows($result) == 1) { // Valid user ID, display the form.
        ...
        print '
        <form action="delete-route.php?id=' . $id . '" method="post">
            <fieldset class="border border-secondary rounded p-3 mb-3">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="number" class="form-control text-center" name="distance" id="distance" value="' . $distance . '" disabled />
                </div>
            </fieldset>
            <p class="fs-5 text-danger">Are you sure you want to delete this route?<br />The action can not be undone.</p>
            ...
            <input type="submit" name="delete-route" value="DELETE ROUTE" class="fs-5 btn btn-success rounded-pill mt-4 py-1 px-4 ms-lg-5" />
        </form>';
    } else {
        print '<p>This page has been accessed in error.</p>';
