...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        // Start updating
        $tariff_in_birr = filter_var($_POST['tariff'], FILTER_SANITIZE_NUMBER_FLOAT);

        // Updates the selected route based on its id
        $query = "UPDATE routes SET tariff=? WHERE route_id=? LIMIT 1";
        $q = mysqli_stmt_init($dbcon);
        mysqli_stmt_prepare($q, $query);
        mysqli_stmt_bind_param($q, 'di', $tariff_in_birr, $id);
        mysqli_stmt_execute($q);

        // Updates the other route using the two infomation
        $third_query = "UPDATE routes SET tariff=? WHERE departure_place=? AND arrival_place=? LIMIT 1";
        $third_q = mysqli_stmt_init($dbcon);
        mysqli_stmt_prepare($third_q, $third_query);
        mysqli_stmt_bind_param($third_q, 'dss', $tariff_in_birr, $second_arrival, $second_departure); // Crafty place switch
        mysqli_stmt_execute($third_q);

        if ((mysqli_stmt_affected_rows($q) == 1) && (mysqli_stmt_affected_rows($third_q) == 1)) { // Update OK
            header('location: ../../../php/route-tasks/successful-tariff-update.php');
    ...
    $q = mysqli_stmt_init($dbcon);
    $query = "SELECT departure_place, arrival_place, distance_in_km, tariff FROM routes WHERE route_id=?";
    mysqli_stmt_prepare($q, $query);
    ...
    if (mysqli_num_rows($result) == 1) { // Valid user ID, display the form.
        ...
        $tariff = htmlspecialchars($row[3], ENT_QUOTES);
        print '
        <form action="assign-tariff.php?id=' . $id . '" method="post">
            <fieldset class="border border-secondary rounded p-3 mb-3">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="number" class="form-control text-center" name="distance" id="distance" min="0" max="2000" value="' . $distance . '" disabled />
                </div>
            </fieldset>
            <div class="input-group mb-3">
                <label for="tariff" class="form-label w-35 pt-1 pe-3 text-end">Tariff (in Birr)</label>
                <input type="number" class="form-control text-center" name="tariff" id="tariff" min="0" max="10000" value="' . $tariff . '" />
            </div>
            <a href="select-route.php">
            ...
            <input type="submit" name="assign-tariff" id="assign-tariff" value="ASSIGN TARIFF" class="fs-5 btn btn-success rounded-pill mt-4 py-1 px-4 ms-lg-5" />
        </form>';
    } else {
        print '<p>This page has been accessed in error.</p>';
