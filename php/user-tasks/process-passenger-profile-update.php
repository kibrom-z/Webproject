...
try {
        ...
        if (empty($errors)) { // If everything's OK
            try {
                ...
                if (empty($errors)) { // The username is unique and the password is correct
                    ...
                    $query = "UPDATE users SET first_name=?, last_name=?, sex=?, phone=?, email=?, username=?, bank_acc_number=? WHERE user_id=? LIMIT 1";
                    $q = mysqli_stmt_init($dbcon);
                    mysqli_stmt_prepare($q, $query);
                    mysqli_stmt_bind_param($q, 'sssssssi', $first_name, $last_name, $sex, $phone, $email, $username, $bank, $id);
    ...
    $q = mysqli_stmt_init($dbcon);
    $query = "SELECT first_name, last_name, sex, phone, email, username, bank_acc_number FROM users WHERE user_id=?";
    mysqli_stmt_prepare($q, $query);
    ...
    if (mysqli_num_rows($result) == 1) { // Valid user ID, display the form.
        ...
        $bankAccount = htmlspecialchars($row[6], ENT_QUOTES);
        ...
        print '
                ...
                <div class="input-group mb-3">
                    <label for="bank-acc" class="form-label w-35 pt-1 pe-3 text-end">Bank account number</label>
                    <input type="text" class="form-control ps-4" pattern="[0-9]{10,15}" title="Must be 10 to 15 Numbers Long" name="bank-acc" id="bank-acc" value="' . $bankAccount . '" />
                </div>
                <div class="input-group mb-3">
