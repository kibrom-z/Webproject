...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) { // If everything's OK
      ...
      $retrieve_query = "SELECT reservation_id FROM reservations WHERE first_name=? AND email=? AND phone=?";
      $prepared = mysqli_stmt_init($dbcon);
      mysqli_stmt_prepare($prepared, $retrieve_query);
      mysqli_stmt_bind_param($prepared, "sss", $first_name, $email, $phone);
  ...
  if (mysqli_num_rows($result) == 1) { // Valid user ID, display the form.
    ...
    print '
        <input type="submit" name="confirm" value="UPATE DETAIL" class="fs-5 btn btn-success rounded-pill mt-5 py-1 w-50" id="confirm-button" />
    </form>
        ';
  } else {
    print '<p>This page has been accessed in error.</p>';
