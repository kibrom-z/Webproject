...
if (strcasecmp($departure_place, $arrival_place) == 0) { // Compares case insensitively
    $errors[] = 'Departure and arrival place can not be the same.';
}
$departure_date = filter_var($_POST['dep-date'], FILTER_SANITIZE_STRING);
if (empty($departure_date)) {
    $errors[] = 'You forgot to select departure date.';
}
$return_date = filter_var($_POST['ret-date'], FILTER_SANITIZE_STRING);
if (empty($return_date)) {
    $errors[] = 'You forgot to select return date.';
}
if ($departure_date == $return_date) {
    $errors[] = 'Departure and return date can not be the same.';
}
...
if (empty($errors)) { // If everything's OK
    try {
        ...
        if (mysqli_num_rows($result) == 1) {
            $first_route_id = htmlspecialchars($row[0], ENT_QUOTES);
        } else {
        ...
        mysqli_stmt_bind_param($prepared, "ss", $departure_date, $first_route_id);
        mysqli_stmt_execute($prepared);
        $result = mysqli_stmt_get_result($prepared);

        if (mysqli_num_rows($result) == 0) {
            header('location: first-trip.php?id=0');
        } else {
            $_SESSION['departure-place'] = $departure_place;
            $_SESSION['arrival-place'] = $arrival_place;
            $_SESSION['return-date'] = $return_date;
            $_SESSION['adult-number'] = $adult_number;
            $_SESSION['manipulable-adult-number'] = $adult_number;
            $_SESSION['child-number'] = $child_number;
            $_SESSION['first-route-id'] = $first_route_id;
            $_SESSION['first-trip-ids'] = array(); // If multiple trips are found
            $_SESSION['reservation-ids'] = array();

            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                $_SESSION['first-trip-ids'][] = $row['trip_id'];
            }
            header('location: first-trip.php?id=1');
        }
    } catch (Exception $e) {
