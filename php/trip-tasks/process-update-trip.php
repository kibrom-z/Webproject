...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query = "UPDATE trips SET departure_date=?, departure_time=? WHERE trip_id=? LIMIT 1";
        $q = mysqli_stmt_init($dbcon);
        mysqli_stmt_prepare($q, $query);
        mysqli_stmt_bind_param($q, 'ssi', $departure_date, $departure_time, $id);
        mysqli_stmt_execute($q);

        if (mysqli_stmt_affected_rows($q) == 1) { // Update OK
            header('location: ../../../php/trip-tasks/successful-trip-update.php');
    ...
    if (mysqli_num_rows($result) == 1) { // Valid user ID, display the form.
        ...
        // Print the form
        print '
        <form action="update-trip.php?id=' . $id . '" method="post">
        ...
        <div class="input-group mb-3">
            ...
            <input type="date" class="form-control text-center" name="dep-date" id="dep-date" value="' . $date . '" required />
        </div>
        ...
        <div class="input-group mb-3">
            ...
            <input type="time" class="form-control text-center" name="dep-time" id="dep-time" value="' . $time . '" required />
        </div>
        ...
        <input type="submit" name="update-trip" id="update-trip" value="UPDATE TRIP" class="fs-5 btn btn-success rounded-pill mt-4 py-1 px-5 ms-lg-5" />
        </form>';
    } else {
        print '<p>This page has been accessed in error.</p>';
